...
Per-project rules live in each project's own `CLAUDE.md`.
...
language for bot, scraper, and extension. The other three are Python; for
numeral-translator-ai this is forced (`num2words` has no real JS equivalent).

---

## Critical rules (all four projects)
...
Each project has its own four sub-agents, listed in that project's `AGENTS.md`.
Naming convention is `<project-prefix>-<role>`:
...
All four projects are **spec-only** as of 2026-08-08 — a `REQUIREMENTS.md` and
nothing else. Each doc ends with open questions that are still open. Read the
project's `REQUIREMENTS.md` before proposing any implementation.
